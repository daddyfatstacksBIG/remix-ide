...
var sources = [
  {
    "localhost/folder1/contract2.sol": {
      content:
        "contract test2 { function get () public returns (uint) { return 11; }}"
    }
  },
  {
...
function runTests(browser, testData) {
  ...
  if (browserName === "safari" || browserName === "internet explorer") {
    console.log(
      "do not run remixd test for " +
        browserName +
        ": sauce labs doesn't seems to handle websocket"
    );
    browser.end();
    return;
  }
  if (browserName === "firefox") {
    console.log(
      "do not run remixd test for " + browserName + ": TODO to reenable later"
    );
    browser.end();
    return;
  }
  browser
    ...
    .perform(function(done) {
      testImportFromRemixd(browser, () => {
        done();
      });
    })
    .clickLaunchIcon("fileExplorers")
    .click('[data-path="localhost"]') // collapse and expand
    ...
    .waitForElementVisible(
      '[data-path="localhost/folder1/renamed_contract_' + browserName + '.sol"]'
    ) // check if renamed file is preset
    .waitForElementNotPresent(
      '[data-path="localhost/folder1/contract_' + browserName + '.sol"]'
