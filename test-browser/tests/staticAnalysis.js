...
function runTests(browser) {
  browser
    ...
    .testContracts("Untitled.sol", sources[0]["browser/Untitled.sol"], [
      "TooMuchGas",
      "test1",
      "test2"
    ])
    ...
    .waitForElementPresent(
      ...
      function() {
        listSelectorContains(
          [
            "browser/Untitled.sol:3:33:Use of tx.origin",
  ...
  browser.execute(
    ...
    function(result) {
      ...
      for (var k in textsToFind) {
        console.log(
          "testing " + result.value[k] + " against " + textsToFind[k]
        );
        browser.assert.equal(
          result.value[k].indexOf(textsToFind[k]) !== -1,
          true
        );
      }
      callback();
