...
Please make a backup of your contracts and start using http://remix.ethereum.org`);
  ...
  const { eventsDecoder, txlistener } = makeUdapp(
    udapp,
    compilersArtefacts,
    domEl => mainview.getTerminal().logHtml(domEl)
  );
  ...
  if (!loadedFromGist) {
    ...
    self._components.filesProviders["browser"].resolveDirectory(
      "/",
      (error, filesList) => {
        ...
        if (Object.keys(filesList).length === 0) {
          ...
          fileManager.setFile(
            examples.ballot_test.name,
            examples.ballot_test.content
          );
        }
      }
    );
