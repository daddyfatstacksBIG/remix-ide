...
UniversalDAppUI.prototype.renderInstance = function(
  contract,
  address,
  contractName
) {
...
UniversalDAppUI.prototype.renderInstanceFromABI = function(
  contractABI,
  address,
  contractName
) {
  var self = this;
  address =
    (address.slice(0, 2) === "0x" ? "" : "0x") + address.toString("hex");
  ...
  var title = yo`
    <div class="${css.title} alert alert-secondary p-2">
      <button class="btn ${css.titleExpander}" onclick="${e => {
    toggleClass(e);
  }}">
      ...
      <div class="input-group ${css.nameNbuts}">
        ...
        <div class="btn-group">
          <button class="btn p-1 btn-secondary">${copyToClipboard(
            () => address
          )}</button>
        </div>
      </div>
  ...
  if (fallback) {
    contractActionsWrapper.appendChild(
      this.getCallButton({
        funABI: fallback,
        address: address,
...
UniversalDAppUI.prototype.getCallButton = function(args) {
  ...
  function clickButton(valArr, inputsValues) {
    ...
    if (!args.funABI.constant) {
      logMsg = `transact to ${args.contractName}.${
        args.funABI.name ? args.funABI.name : "(fallback)"
      }`;
    } else {
      logMsg = `call to ${args.contractName}.${
        args.funABI.name ? args.funABI.name : "(fallback)"
      }`;
    }
    ...
    var confirmationCb = (
      ...
      var amount = executionContext
        .web3()
        .fromWei(typeConversion.toInt(tx.value), "ether");
      var content = confirmDialog(
        tx,
        amount,
        gasEstimation,
        self.udapp,
            ...
            var fee = executionContext
              ...
              .mul(
                executionContext
                  .web3()
                  .toBigNumber(
                    executionContext.web3().toWei(gasPrice.toString(10), "gwei")
                  )
              );
            txFeeText =
              " " +
              executionContext.web3().fromWei(fee.toString(10), "ether") +
              " Ether";
            priceStatus = true;
          } catch (e) {
            txFeeText =
        ...
        cb => {
          executionContext.web3().eth.getGasPrice((error, gasPrice) => {
            ...
            try {
              var gasPriceValue = executionContext
                .web3()
                .fromWei(gasPrice.toString(10), "gwei");
          ...
          fn: () => {
            ...
            if (!content.gasPriceStatus) {
              ...
              var gasPrice = executionContext
                .web3()
                .toWei(content.querySelector("#gasprice").value, "gwei");
    ...
    txFormat.buildData(
      ...
      false,
      args.funABI,
      args.funABI.type !== "fallback" ? value : "",
      (error, data) => {
        if (!error) {
          if (!args.funABI.constant) {
          ...
          self.udapp.callFunction(
            ...
            (error, txResult) => {
              if (!error) {
                ...
                if (lookupOnly) {
                  var decoded = decodeResponseToTreeView(
                    executionContext.isVM()
                      ? txResult.result.vm.return
                      : ethJSUtil.toBuffer(txResult.result),
                    args.funABI
                  );
                  outputCb(decoded);
                }
  ...
  var multiParamManager = new MultiParamManager(
    lookupOnly,
    args.funABI,
    (valArray, inputsValues, domEl) => {
      clickButton(valArray, inputsValues, domEl);
    },
    self.udapp.getInputs(args.funABI)
  );

  var contractActionsContainer = yo`<div class="${
    css.contractActionsContainer
  }" >${multiParamManager.render()}</div>`;
