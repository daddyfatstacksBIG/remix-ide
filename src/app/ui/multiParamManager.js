...
class MultiParamManager {
  ...
  render() {
    ...
    let funcButton = yo`<button onclick=${() => onClick()} class="${
      css.instanceButton
    } btn btn-sm">${title}</button>`;
    this.contractActionsContainerSingle = yo`
    <div class="${css.contractActionsContainerSingle}" >
      ${funcButton}
      ${this.basicInputField}
      <i class="fas fa-angle-down ${css.methCaret}" onclick=${() =>
      this.switchMethodViewOn()} title=${title} ></i>
    </div>`;
    ...
    var expandedButton = yo`<button onclick=${() => {
      multiOnClick();
    }} class="${css.instanceButton}"></button>`;

    this.contractActionsContainerMulti = yo`<div class="${
      css.contractActionsContainerMulti
    }" >
    ...
    if (this.lookupOnly) {
      ...
      this.funABI.stateMutability === "payable" ||
      this.funABI.payable === true
    ) {
    ...
    if (this.funABI.inputs && this.funABI.inputs.length > 0) {
      ...
      contractProperty.classList.add(css.hasArgs);
      this.contractActionsContainerSingle.querySelector("i").style.visibility =
        "hidden";
    } else {
      this.contractActionsContainerSingle.querySelector("i").style.visibility =
        "hidden";
