          ...
          compilerImport.import(
            ...
            (error, content, cleanUrl, type, url) => {
              if (error) {
                ...
                fileProviders["browser"].addExternal(
                  type + "/" + cleanUrl,
                  content,
                  url
                );
    ...
    let container = yo`<div class="${css.homeContainer} bg-light">
        ...
        <div class="alert alert-info clearfix ${css.thisJumboton}">
          ...
          <div class="${css.jumboBtnContainer} px-5">
            <button class="btn btn-primary btn-lg mx-3" href="#" onclick=${() =>
              learnMore()} role="button">Learn more</button>
            <button class="btn btn-secondary btn-lg" onclick=${() =>
              switchToPreviousVersion()}>Use previous version</button>
          </div>
          ...
          <div class="mb-5">
            ...
            <div class="${css.enviroments} pt-2">
              <button class="btn btn-lg btn-secondary mr-3" onclick=${() =>
                startSolidity()}>Solidity</button>
              <button class="btn btn-lg btn-secondary mr-3" onclick=${() =>
                startVyper()}>Vyper</button>
              <button class="btn btn-lg btn-secondary mr-3" onclick=${() =>
                startWorkshop()}>Workshops</button>
            </div>
          </div>
          <div class="file">
            <h4>File</h4>
            <p class="mb-1 ${css.text}" onclick=${() =>
      createNewFile()}>New File</p>
            <p class="mb-1">
              <label class="${css.labelIt} ${css.text}">
                Open Files
                <input title="open file" type="file" onchange="${event => {
                  event.stopPropagation();
                  let fileExplorer = globalRegistry.get("fileexplorer/browser")
                    .api;
            ...
            <p class="mb-1 ${css.text}" onclick=${() =>
      connectToLocalhost()}>Connect to Localhost</p>
            <p class="mb-1">Import From:</p>
            <div class="btn-group">
              <button class="btn btn-sm btn-secondary" onclick="${() =>
                importFromGist()}">Gist</button>
              <button class="btn btn-sm btn-secondary" onclick="${() =>
                load("Github", "github URL", [
              ...
              <button class="btn btn-sm btn-secondary" onclick="${() =>
                load("Swarm", "bzz-raw URL", [
                  "bzz-raw://<swarm-hash>"
                ])}">Swarm</button>
              <button class="btn btn-sm btn-secondary" onclick="${() =>
                load("Ipfs", "ipfs URL", ["ipfs://<ipfs-hash>"])}">Ipfs</button>
              <button class="btn btn-sm btn-secondary" onclick="${() =>
                load("Https", "http/https raw content", [
              ...
              <button class="btn btn-sm btn-secondary" onclick="${() =>
                load(
                  ...
                  yo`<span>please checkout <a class='text-primary' href="https://github.com/Crypto-Punkers/resolver-engine" target='_blank'>https://github.com/Crypto-Punkers/resolver-engine</a> for more information</span>`
      ...
      startDebugger();
            ...
            <p class="mb-1">
              <button onclick=${() => {
                startPluginManager();
              }} class="btn btn-sm btn-secondary ${css.seeAll}">
                See all Plugins
