            ...
            provider.get(fileName, (error, content) => {
              if (!error) {
                ...
                self.networks.forEach(network => {
                  deploy[network] = self._syncContext(
                    contract,
                    deploy[network] || {}
                  );
                });
  ...
  deployMetadataOf(contractName) {
    return new Promise((resolve, reject) => {
      ...
      if (provider && path) {
        ...
        reject(
          `Please select the folder in the file explorer where the metadata of ${contractName} can be found`
        );
      }
    });
  }
