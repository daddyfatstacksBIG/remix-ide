...
class FileProvider {
  ...
  set(path, content, cb) {
    ...
    if (
      exists &&
      window.remixFileSystem.readFileSync(unprefixedpath, "utf8") === content
    )
      return true;
    ...
    if (!exists && unprefixedpath.indexOf("/") !== -1) {
      ...
      paths.forEach(value => {
        ...
        if (!window.remixFileSystem.existsSync(currentCheck)) {
          ...
          this.event.trigger("folderAdded", [
            this._normalizePath(currentCheck)
          ]);
        }
      });
    }
    ...
    if (!exists) {
      this.event.trigger("fileAdded", [
        this._normalizePath(unprefixedpath),
        false
      ]);
    } else {
    ...
    window.remixFileSystem.readdir(path, (error, files) => {
      ...
      if (files) {
        files.forEach(element => {
          ...
          ret[
            absPath.indexOf("/") === 0 ? absPath.replace("/", "") : absPath
          ] = {
