...
class FileManager extends Plugin {
  ...
  init() {
    this._deps = {
      ...
      browserExplorer: this._components.registry.get("fileproviders/browser")
        .api,
      localhostExplorer: this._components.registry.get(
    ...
    this._deps.browserExplorer.event.register(
      "fileRenamed",
      (oldName, newName, isFolder) => {
        this.fileRenamedEvent(oldName, newName, isFolder);
      }
    );
    this._deps.localhostExplorer.event.register(
      "fileRenamed",
      (oldName, newName, isFolder) => {
        this.fileRenamedEvent(oldName, newName, isFolder);
      }
        ...
        return yo`
          <div class="container ml-1">
            <button class="btn btn-primary btn-sm m-1" onclick=${e =>
              acceptFileRewriting(e, toaster)}>
              Accept
            </button>
            <button class="btn btn-primary btn-sm m-1" onclick=${e =>
              cancelFileRewriting(e, toaster)}>
              Cancel
            </button>
            <button class="btn btn-primary btn-sm m-1" onclick="${e =>
              saveFileAsCopy(e, toaster)}">
              Save As Copy
            </button>
          </div>
        `;
  ...
  _saveAsCopy(path, content) {
    ...
    if (fileProvider) {
      helper.createNonClashingNameWithPrefix(
        path,
        fileProvider,
        "",
        (error, copyName) => {
    ...
    if (currentFile && this.editor.current()) {
      ...
      if (input) {
        ...
        if (provider) {
          ...
          console.log(
            "cannot save " + currentFile + ". Does not belong to any explorer"
          );
        }
      }
    }
  ...
  setBatchFiles(filesSet, fileProvider, override, callback) {
    ...
    async.each(
      Object.keys(filesSet),
      (file, callback) => {
        if (override) {
          self._deps.filesProviders[fileProvider].set(
            file,
            filesSet[file].content
          );
          self.syncEditor(fileProvider + file);
          return callback();
        }

        helper.createNonClashingName(
          file,
          self._deps.filesProviders[fileProvider],
          (error, name) => {
            if (error) {
              modalDialogCustom.alert(
                "Unexpected error loading the file " + error
              );
