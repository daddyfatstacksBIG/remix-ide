...
function fileExplorer(localRegistry, files, menuItems) {
  ...
  this.files.event.register("fileExternallyChanged", (path, file) => {
    if (
      self._deps.config.get("currentFile") === path &&
      self._deps.editor.currentContent() &&
      self._deps.editor.currentContent() !== file.content
    ) {
      if (this.files.isReadOnly(path))
        return self._deps.editor.setText(file.content);
    ...
    self.ensureRoot(() => {
      ...
      if (currentTree && self.treeView.isExpanded(folderpath)) {
        self.files.resolveDirectory(folderpath, (error, fileTree) => {
          ...
          if (
            self.focusElement &&
            !self.focusElement.classList.contains("bg-secondary")
          ) {
  ...
  self.treeView.event.register("nodeRightClick", function(
    key,
    data,
    label,
    event
    ...
    actions["Delete"] = () => {
      ...
      modalDialogCustom.confirm(
        ...
        () => {
          if (!files.remove(key))
            tooltip(
              `failed to remove ${key}. Make sure the directory is empty before removing it.`
            );
        },
        () => {}
      );
  ...
  self.treeView.event.register("leafRightClick", function(
    key,
    data,
    label,
    event
    ...
    if (!provider.isExternalFolder(key)) {
      actions["Create Folder"] = () =>
        self.createNewFolder(self._deps.fileManager.extractPathOf(key));
      actions["Rename"] = () => {
        if (self.files.isReadOnly(key)) {
          return tooltip(
            "cannot rename file. " +
  ...
  function editModeOff(event) {
        ...
        files.exists(newPath, (error, exist) => {
          if (error)
            return modalDialogCustom.alert(
              "Unexpected error while renaming: " + error
            );
          if (!exist) {
    ...
    if (
      (event.type === "blur" || event.which === 13) &&
      label.getAttribute("contenteditable")
    ) {
...
fileExplorer.prototype.packageFiles = function(
  filesProvider,
  directory,
  callback
) {
  ...
  modalDialogCustom.prompt(
    ...
    input => {
      helper.createNonClashingName(input, self.files, (error, newName) => {
        if (error)
          return modalDialogCustom.alert(
            "Failed to create file " + newName + " " + error
          );
        const currentPath = !parentFolder
          ? self._deps.fileManager.currentPath()
          : parentFolder;
        newName = currentPath
          ? currentPath + "/" + newName
          : self.files.type + "/" + newName;
  ...
  modalDialogCustom.prompt(
    ...
    input => {
      const currentPath = !parentFolder
        ? self._deps.fileManager.currentPath()
        : parentFolder;
      let newName = currentPath
        ? currentPath + "/" + input
        : self.files.type + "/" + input;
    ...
    items = this.menuItems.map(({ action, title, icon }) => {
      if (action === "uploadFile") {
        ...
        <span onclick=${event => {
          event.stopPropagation();
          this[action]();
        }} class="newFile ${icon} ${css.newFile}" title=${title}></span>
        `;
      }
    });
  }
