...
module.exports = class RemixDProvider {
    ...
    remixd.event.register("notified", data => {
      if (data.scope === "sharedfolder") {
        if (data.name === "created") {
          this.init(() => {
            this.event.trigger("fileAdded", [
              this.type + "/" + data.value.path,
          ...
          this.init(() => {
            this.event.trigger("fileRemoved", [
              this.type + "/" + data.value.path
            ]);
          });
  ...
  set(path, content, cb) {
    ...
    this._remixd.call(
      "sharedfolder",
      "set",
      { path: unprefixedpath, content: content },
      (error, result) => {
  ...
  rename(oldPath, newPath, isFolder) {
    ...
    this._remixd.call(
      "sharedfolder",
      "rename",
      { oldPath: unprefixedoldPath, newPath: unprefixednewPath },
      (error, result) => {
        if (error) {
          console.log(error);
...
function remixapi(remixd, self) {
  const read = (path, callback) => {
    ...
    remixd.call("sharedfolder", "get", { path }, (error, content) =>
  ...
  const write = (path, content, callback) => {
    path = "" + (path || "");
    path = pathtool.join("./", path);
    remixd.call("sharedfolder", "set", { path, content }, (error, result) =>
  ...
  const remove = (path, callback) => {
    path = "" + (path || "");
    path = pathtool.join("./", path);
    remixd.call("sharedfolder", "remove", { path }, (error, result) =>
      callback(error, result)
    );
  };
  const dir = (path, callback) => {
    ...
    remixd.call(
      ...
      "resolveDirectory",
      { path },
      (error, filesList) => callback(error, filesList)
    );
  };
  const exit = () => {
    remixd.close();
