...
class SettingsUI {
  constructor(settings, networkModule) {
    ...
    this.settings.event.register(
      "transactionExecuted",
      (error, from, to, data, lookupOnly, txResult) => {
        if (error) return;
    ...
    this._deps.config.events.on(
      "settings/personal-mode_changed",
      this.onPersonalChange.bind(this)
    );
  ...
  updateAccountBalances() {
    ...
    accounts.each((index, account) => {
      this.settings.getAccountBalanceForAddress(
        account.value,
        (err, balance) => {
          if (err) return;
  ...
  render() {
    ...
    var environmentEl = yo`
      <div class="${css.crow}">
        ...
        <div class=${css.environment}>
          ...
          <a href="https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md" target="_blank"><i class="${
            css.infoDeployAction
          } fas fa-info"></i></a>
        </div>
      </div>
    `;
    ...
    const accountEl = yo`
      <div class="${css.crow}">
        <div class="${css.col1_1}">
          Account
          <span id="remixRunPlusWraper" title="Create a new account" onload=${this.updatePlusButton.bind(
            this
          )}>
            <i id="remixRunPlus" class="fas fa-plus-circle ${
              css.icon
            }" aria-hidden="true" onclick=${this.newAccount.bind(this)}"></i>
          </span>
        </div>
        <div class=${css.account}>
          ...
          ${copyToClipboard(
            () => document.querySelector("#runTabView #txorigin").value
          )}
          <i id="remixRunSignMsg" class="fas fa-edit ${
            css.icon
          }" aria-hidden="true" onclick=${this.signMessage.bind(
      this
    )} title="Sign a message using this account key"></i>
        </div>
      </div>
    `;
  ...
  setDropdown(selectExEnv) {
    ...
    selectExEnv.addEventListener("change", event => {
      ...
      this.settings.changeExecutionContext(
        context,
        () => {
          const modal = modalDialogCustom.confirm(
            ...
            () => {
              ...
              modalDialogCustom.prompt(
                ...
                "http://localhost:8545",
                target => {
                  this.settings.setProviderFromEndpoint(
                    target,
                    context,
                    alertMsg => {
      ...
      case "injected":
        {
          ...
          plusTitle.title =
            "Unfortunately it's not possible to create an account using injected web3. Please create the account directly from your provider (i.e metamask or other of the same type).";
        }
        break;
      case "vm":
        {
  ...
  newAccount() {
    this.settings.newAccount(
      cb => {
        modalDialogCustom.promptPassphraseCreation(
          (error, passphrase) => {
            if (error) {
  ...
  signMessage() {
    this.settings.getAccounts((err, accounts) => {
      ...
      if (
        !$txOrigin.selectedOptions[0] &&
        (this.settings.isInjectedWeb3() || this.settings.isWeb3Provider())
      ) {
        return addTooltip(
      ...
      var promptCb = passphrase => {
        const modal = modalDialogCustom.promptMulti(
          signMessageDialog,
          message => {
            this.settings.signMessage(
              message,
              account,
              passphrase,
              (err, msgHash, signedData) => {
                if (err) {
    ...
    this.settings.getAccounts((err, accounts) => {
      ...
      for (var loadedaddress in this.loadedAccounts) {
        if (accounts.indexOf(loadedaddress) === -1) {
          txOrigin.removeChild(
            txOrigin.querySelector('option[value="' + loadedaddress + '"]')
          );
      ...
      for (var i in accounts) {
        ...
        if (!this.loadedAccounts[address]) {
          txOrigin.appendChild(
            yo`<option value="${address}" >${address}</option>`
          );
