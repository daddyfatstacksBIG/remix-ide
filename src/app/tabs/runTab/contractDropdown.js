...
class ContractDropdownUI {
  ...
  listenToEvents() {
    this.dropdownLogic.event.register(
      "newlyCompiled",
      (success, data, source, compiler, compilerFullName, file) => {
        if (!document.querySelector(`.${css.contractNames.classNames[0]}`))
          return;
        var contractNames = document.querySelector(
          `.${css.contractNames.classNames[0]}`
        );
        ...
        if (success) {
          ...
          this.dropdownLogic
            ...
            .forEach(contract => {
              contractNames.appendChild(
                yo`<option value="${contract.name}" compiler="${compilerFullName}">${contract.name} - ${file}</option>`
              );
            });
        } else {
        ...
        if (success) {
          ...
          document
            ...
            .classList.remove(css.contractNamesError);
        } else {
          this.compFails.style.display = "block";
          document
            .querySelector(`.${css.contractNames}`)
            .classList.add(css.contractNamesError);
        }
      }
    );
    ...
    this.dropdownLogic.event.register(
      "currentFileChanged",
      this.changeCurrentFile.bind(this)
    );
  }
  ...
  render() {
    ...
    this.atAddress = yo`<button class="${
      css.atAddress
    } btn btn-sm btn-info" disabled id="runAndDeployAtAdressButton" onclick=${this.loadFromAddress.bind(
      this
    )}>At Address</button>`;
    this.atAddressButtonInput = yo`<input class="${css.input} ${
      css.ataddressinput
    } ataddressinput form-control" placeholder="Load contract from Address" title="address of contract" oninput=${this.atAddressChanged.bind(
      this
    )} />`;
    ...
    `;
    this.selectContractNames.addEventListener(
      "change",
      this.setInputParamsPlaceHolder.bind(this)
    );
    this.setInputParamsPlaceHolder();
    if (!this.el) {
      this.el = el;
  ...
  setInputParamsPlaceHolder() {
    ...
    if (
      this.selectContractNames.selectedIndex < 0 ||
      this.selectContractNames.children.length <= 0
    ) {
  ...
  getSelectedContract() {
    var contract = this.selectContractNames.children[
      this.selectContractNames.selectedIndex
    ];
    var contractName = contract.getAttribute("value");
    var compilerAtributeName = contract.getAttribute("compiler");

    return this.dropdownLogic.getSelectedContract(
      contractName,
  ...
  createInstance(args) {
    ...
    if (selectedContract.bytecodeObject.length === 0) {
      return modalDialogCustom.alert(
    ...
    var continueCb = (error, continueTxExecution, cancelCb) => {
      if (error) {
        ...
        modalDialog(
          ...
          yo`<div>Gas estimation errored with the following message (see below).
    ...
    var promptCb = (okCb, cancelCb) => {
      modalDialogCustom.promptPassphrase(
    ...
    var finalCb = (error, contractObject, address) => {
      ...
      this.event.trigger("newContractInstanceAdded", [
        contractObject,
        address,
        contractObject.name
      ]);
    ...
    if (selectedContract.isOverSizeLimit()) {
      return modalDialog(
        ...
        yo`<div>Contract creation initialization returns data with length of more than 24576 bytes. The deployment will likely fails. <br>
      More info: <a href="https://github.com/ethereum/EIPs/blob/master/EIPS/eip-170.md" target="_blank">eip-170</a>
      </div>`,
        {
          label: "Force Send",
          fn: () => {
            this.dropdownLogic.forceSend(
              selectedContract,
              args,
              continueCb,
              promptCb,
        ...
        },
        {
          ...
          fn: () => {
            this.logCallback(
              `creation of ${selectedContract.name} canceled by user.`
            );
          }
        }
      );
  ...
  loadFromAddress() {
    ...
    this.dropdownLogic.loadContractFromAddress(
      address,
      cb => {
      ...
      (error, loadType, abi) => {
        ...
        this.event.trigger("newContractInstanceAdded", [
          ...
          address,
          this.selectContractNames.value
        ]);
      }
    );
  }
