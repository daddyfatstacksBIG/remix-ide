...
class DropdownLogic {
  ...
  listenToCompilationEvents() {
    let broadcastCompilationResult = (file, source, languageVersion, data) => {
      ...
      this.event.trigger("newlyCompiled", [
        true,
        data,
        source,
        compiler,
  ...
  loadContractFromAddress(address, confirmCb, cb) {
    ...
    if (
      /[a-f]/.test(address) &&
      /[A-F]/.test(address) &&
      !ethJSUtil.isValidChecksumAddress(address)
    ) {
  ...
  getSelectedContract(contractName, compilerAtributeName) {
    ...
    return {
      ...
      getConstructorInputs: () => {
        var constructorInteface = txHelper.getConstructorInterface(
          contract.object.abi
        );
        return txHelper.inputParametersDeclarationToString(
          constructorInteface.inputs
        );
      },
      isOverSizeLimit: () => {
  ...
  fromWei(value, doTypeConversion, unit) {
    if (doTypeConversion) {
      return executionContext
        .web3()
        .fromWei(typeConversion.toInt(value), unit || "ether");
    }
    ...
    return executionContext
      ...
      .mul(
        executionContext
          .web3()
          .toBigNumber(
            executionContext.web3().toWei(gasPrice.toString(10), unit || "gwei")
          )
      );
  }
  ...
  createContract(
    ...
    var confirmationCb = (
      ...
      modalDialog(
        ...
        content,
        {
          label: "Confirm",
          fn: () => {
            this.config.setUnpersistedProperty(
    ...
    this.udapp.createContract(
      ...
      (error, txResult) => {
        if (error) {
          return finalCb(
            `creation of ${selectedContract.name} errored: ${error}`
          );
        }
        ...
        var address = isVM
          ? txResult.result.createdAddress
          : txResult.result.contractAddress;
        ...
        content,
        {
          ...
          fn: () => {
            ...
            if (!content.gasPriceStatus) {
              ...
              var gasPrice = this.toWei(
                content.querySelector("#gasprice").value,
                "gwei"
              );
              continueTxExecution(gasPrice);
            }
  ...
  async forceSend(
    ...
    try {
      contractMetadata = await this.runView.call(
        "compilerMetadata",
        "deployMetadataOf",
    ...
    if (
      !contractMetadata ||
      (contractMetadata && contractMetadata.autoDeployLib)
    ) {
      return txFormat.buildData(
        selectedContract.name,
        selectedContract.object,
        this.compilersArtefacts["__last"].getData().contracts,
        true,
        constructor,
        args,
        (error, data) => {
    ...
    if (Object.keys(selectedContract.bytecodeLinkReferences).length)
      statusCb(
        `linking ${JSON.stringify(
          selectedContract.bytecodeLinkReferences,
          null,
          "\t"
        )} using ${JSON.stringify(contractMetadata.linkReferences, null, "\t")}`
      );
