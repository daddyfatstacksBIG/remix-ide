...
module.exports = class TestTab extends ViewPlugin {
  ...
  listenToEvents() {
    this.filePanel.event.register("newTestFileCreated", file => {
      ...
      var test = yo`<label class="singleTestLabel"><input class="singleTest" onchange=${e =>
        this.toggleCheckbox(
          e.target.checked,
          file
        )} type="checkbox" checked="true">${file}</label>`;
  ...
  listTests() {
    return this.data.allTests.map(
      test =>
        yo`<label class="singleTestLabel"><input class="singleTest" onchange =${e =>
          this.toggleCheckbox(
            e.target.checked,
            test
          )} type="checkbox" checked="true">${test} </label>`
    );
  }

  toggleCheckbox(eChecked, test) {
    if (!this.data.selectedTests) {
      this.data.selectedTests = this._view.el.querySelectorAll(
        ".singleTest:checked"
      );
    }
    let selectedTests = this.data.selectedTests;
    selectedTests = eChecked
      ? [...selectedTests, test]
      : selectedTests.filter(el => el !== test);
  ...
  testCallback(result) {
    ...
    if (result.type === "contract") {
      this.testsOutput.appendChild(
        yo`<div class=${css.outputTitle}>${result.filename} (${result.value})</div>`
      );
    } else if (result.type === "testPass") {
      this.testsOutput.appendChild(
        yo`<div class="${css.testPass} ${css.testLog} bg-success">✓ (${result.value})</div>`
      );
    } else if (result.type === "testFailure") {
      this.testsOutput.appendChild(
        yo`<div class="${css.testFailure} ${css.testLog} bg-danger">✘ (${result.value})</div>`
      );
    }
  }
  ...
  updateFinalResult(_errors, result, filename) {
    ...
    this.testsSummary.appendChild(
      yo`<div class=${css.summaryTitle}> ${filename} </div>`
    );
    if (result.totalPassing > 0) {
      this.testsSummary.appendChild(
        yo`<div class="text-success" >${result.totalPassing} passing (${result.totalTime}s)</div>`
      );
      this.testsSummary.appendChild(yo`<br>`);
    }
    if (result.totalFailing > 0) {
      this.testsSummary.appendChild(
        yo`<div class="text-danger" >${result.totalFailing} failing</div>`
      );
      this.testsSummary.appendChild(yo`<br>`);
    }
    result.errors.forEach((error, index) => {
      this.testsSummary.appendChild(
        yo`<div class="text-danger" >${error.context} - ${error.value} </div>`
      );
      this.testsSummary.appendChild(
        yo`<div class="${css.testFailureSummary} text-danger" >${error.message}</div>`
      );
      this.testsSummary.appendChild(yo`<br>`);
    });
  }
  ...
  */
  testFromSource(content, path = "browser/unit_test.sol") {
    return new Promise((resolve, reject) => {
      ...
      let currentCompilerUrl =
        this.baseurl + "/" + this.compileTab.getCurrentVersion();
    ...
    this.fileManager
      ...
      .then(content => {
        ...
        remixTests.runTestSources(
          ...
          (_err, result, cb) => {
            this.resultsCallback(_err, result, cb);
          },
          (error, result) => {
  ...
  updateGenerateFileAction(currentFile) {
    let el = yo`<button class="${
      css.generateTestFile
    } btn btn-primary" onclick="${this.testTabLogic.generateTestFile.bind(
      this.testTabLogic
  ...
  updateRunAction(currentFile) {
    let el = yo`<button id="runTestsTabRunAction" class="${
      css.runButton
    } btn btn-primary"  onclick="${this.runTests.bind(
      this
    )}">Run Tests</button>`;
    ...
    if (!currentFile || !isSolidityActive) {
      ...
      if (!isSolidityActive)
        el.setAttribute("title", "The solidity module should be activated");
    }
  ...
  updateTestFileList(tests) {
    const testsMessage =
      tests && tests.length ? this.listTests() : "No test file available";
