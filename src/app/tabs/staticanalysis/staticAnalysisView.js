    ...
    <div class="${css.analysis}">
      <div class="${css.buttons}">
          <div class="${css.buttonsInner}">
            <button class="${
              css.buttonRun
            } btn btn-sm btn-primary" onclick="${function() {
    self.run();
  }}" >Run</button>
...
staticAnalysisView.prototype.selectedModules = function() {
  ...
  var selected = this.view.querySelectorAll(
      ...
      results.map(function(result, i) {
        result.report.map(function(item, i) {
          ...
          if (item.location) {
            ...
            location =
              Object.keys(self.lastCompilationResult.contracts)[file] +
              ":" +
          ...
          warningCount++;
          var msg = yo`<span>${location} ${item.warning} ${
            item.more
              ? yo`<span><br><a href="${item.more}" target="blank">more</a></span>`
              : yo`<span></span>`
          }</span>`;
...
staticAnalysisView.prototype.checkModule = function(event) {
  let selected = this.view.querySelectorAll(
    '[name="staticanalysismodule"]:checked'
  );
...
staticAnalysisView.prototype.renderModules = function() {
  var self = this;
  var groupedModules = utils.groupBy(
    preProcessModules(self.runner.modules()),
    "categoryId"
  );
