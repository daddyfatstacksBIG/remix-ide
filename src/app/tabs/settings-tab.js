...
module.exports = class SettingsTab extends ViewPlugin {
    ...
    if (themes) {
      return yo`<div class="card-text themes-container">
        ${themes.map(aTheme => {
          ...
          }} class="align-middle form-check-input" name="theme" id="${
            aTheme.name
          }"   >
          <label class="form-check-label" for="${aTheme.name}">${
            aTheme.name
          } (${aTheme.quality})</label>
        </div>`;
          if (this._deps.themeModule.active === aTheme.name)
  ...
  render() {
    ...
    var gistAddToken = yo`<input class="${
      css.savegisttoken
    ...
    var gistRemoveToken = yo`<input class="btn btn-sm btn-primary" id="removegisttoken" onclick=${() => {
      gistAccessToken.value = "";
      this.config.set("settings/gist-access-token", "");
      tooltip("Access token removed");
    }} value="Remove" type="button">`;
    this._view.gistToken = yo`<div class="${
      css.checkboxText
    }">${gistAccessToken}${copyToClipboard(() =>
      this.config.get("settings/gist-access-token")
    )}${gistAddToken}${gistRemoveToken}</div>`;
    this._view.optionVM = yo`<input onchange=${onchangeOption} class="align-middle form-check-input" id="alwaysUseVM" type="checkbox">`;
    if (this.config.get("settings/always-use-vm") === undefined)
      this.config.set("settings/always-use-vm", true);
      ...
      <div class="${css.info} card">
        <div class="card-body">
          ...
          <div class="form-check ${css.frow}">
            ...
            <label class="form-check-label align-middle" for="personal"> ${
              this._view.warnPersonalMode
            } Enable Personal Mode for web3 provider. ${warnText}></label>
          </div>
        </div>
      </div>
      `;
    ...
    function onchangeGenerateContractMetadata(event) {
      self.config.set(
        ...
        !self.config.get("settings/generate-contract-metadata")
      );
    }
    function onchangeOption(event) {
      self.config.set(
        "settings/always-use-vm",
        !self.config.get("settings/always-use-vm")
      );
    }
    function onchangePersonal(event) {
      self.config.set(
        "settings/personal-mode",
        !self.config.get("settings/personal-mode")
      );
    }
