...
class CompilerContainer {
  constructor(compileTabLogic, editor, config, queryParams) {
    ...
    this.data = {
      ...
      defaultVersion: "soljson-v0.5.11+commit.c082d0b4.js", // this default version is defined: in makeMockCompiler (for browser test) and in package.json (downloadsolc_root) for the builtin compiler
  ...
  listenToEvents() {
    ...
    this.compileTabLogic.compiler.event.register(
      "compilationDuration",
      speed => {
    ...
    this.compileTabLogic.compiler.event.register("loadingCompiler", () => {
      if (!this._view.compileIcon) return;
      this._view.compileIcon.setAttribute(
        "title",
  ...
  compilationButton(name) {
    ...
    <div class="${css.compilerArticle}">
      <button class="btn btn-primary btn-block ${
        name === "" ? "disabled" : ""
      }" title="Compile" onclick="${this.compile.bind(this)}">
  ...
  render() {
    this.compileTabLogic.compiler.event.register("compilerLoaded", version =>
    ...
    this._view.autoCompile = yo`<input class="${
      css.autocompile
    }" onchange=${this.updateAutoCompile.bind(
      this
    )} id="autoCompile" type="checkbox" title="Auto compile">`;
    this._view.hideWarningsBox = yo`<input class="${
      css.autocompile
    }" onchange=${this.hideWarnings.bind(
      this
    )} id="hideWarningsBox" type="checkbox" title="Hide warnings">`;
    if (this.data.autoCompile)
      this._view.autoCompile.setAttribute("checked", "");
    if (this.data.hideWarnings)
      this._view.hideWarningsBox.setAttribute("checked", "");

    this._view.optimize = yo`<input onchange=${this.onchangeOptimize.bind(
      this
    )} id="optimize" type="checkbox">`;
    if (this.compileTabLogic.optimize)
      this._view.optimize.setAttribute("checked", "");

    this._view.versionSelector = yo`
      <select onchange="${this.onchangeLoadVersion.bind(
        this
    ...
    this._view.languageSelector = yo`
      <select onchange="${this.onchangeLanguage.bind(
        this
    ...
    this._view.evmVersionSelector = yo`
      <select onchange="${this.onchangeEvmVersion.bind(
        this
    ...
    this._view.includeNightlies = yo`
      <input class="mr-0 ml-1" id="nightlies" type="checkbox" onchange=${this._updateVersionSelector.bind(
        this
      )}>
    `;
  ...
  _updateVersionSelector() {
    ...
    this.data.allversions.forEach(build => {
      const option =
        build.path === this.data.selectedVersion
    ...
    if (this.data.selectedVersion === "builtin") {
      ...
      if (
        this.data.selectedVersion.indexOf("soljson") !== 0 ||
        helper.checkSpecialChars(this.data.selectedVersion)
      ) {
        return console.log(
          "loading " + this.data.selectedVersion + " not allowed"
        );
      }
    ...
    minixhr(`${this.data.baseurl}/list.json`, (json, event) => {
      ...
      if (event.type !== "error") {
        try {
          ...
          if (this.queryParams.get().version)
            selectedVersion = this.queryParams.get().version;
        } catch (e) {
          addTooltip(
        ...
        allversions = [
          { path: "builtin", longVersion: "latest local version" }
        ];
  ...
  scheduleCompilation() {
    ...
    this.data.compileTimeout = window.setTimeout(
      () => this.compileIfAutoCompileOn(),
      this.data.timeout
    );
  }
