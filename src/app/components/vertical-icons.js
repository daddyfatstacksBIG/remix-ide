...
export class VerticalIcons extends Plugin {
  ...
  listenOnStatus(profile) {
    ...
    const fn = status => {
      ...
      if (status.key === undefined)
        throw new Error(`status key should be defined`);
  ...
  resolveClasses(key, type) {
    ...
    switch (key) {
      ...
      case "failed":
        classes +=
          " fas fa-exclamation-triangle text-" + type + " " + css.statusCheck;
        break;
      default: {
   ...
   */
  addActive(name) {
    const themeType = globalRegistry.get("themeModule").api.currentTheme()
      .quality;
    ...
    if (nextActive) {
      ...
      image.style.setProperty(
        "filter",
        `invert(${invert}) grayscale(1) brightness(${brightness}%)`
      );
    }
  }
