...
class PluginManagerComponent extends ViewPlugin {
  ...
  renderItem(name) {
    ...
    const displayName = api.profile.displayName
    ...
    if (
      api.profile.version &&
      api.profile.version.match(/\b(\w*alpha\w*)\b/g)
    ) {
    ...
    const activationButton = isActive
      ? yo`
      <button onclick="${_ =>
        this.appManager.deactivateOne(name)}" class="btn btn-secondary btn-sm">
        Deactivate
      </button>`
      : yo`
      <button onclick="${_ =>
        this.appManager.activateOne(name)}" class="btn btn-success btn-sm">
        Activate
      </button>`;
  ...
  render() {
    ...
    const isFiltered = api =>
      (api.profile.displayName ? api.profile.displayName : api.name)
        .toLowerCase()
    ...
    const { actives, inactives } = this.appManager
      .getAll()
      .filter(isFiltered)
    ...
    const rootView = yo`
      <div id='pluginManager'>
        <header class="form-group ${css.pluginSearch}">
          <input onkeyup="${e =>
            this.filterPlugins(e)}" class="form-control" placeholder="Search">
          <button onclick="${_ =>
            this.openLocalPlugin()}" class="btn btn-sm text-dark border-0 font-weight-bold mt-2">
