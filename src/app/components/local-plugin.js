...
module.exports = class LocalPlugin {
   ...
   */
  open(plugins) {
    this.profile = JSON.parse(localStorage.getItem("plugins/local")) || {
      notifications: {}
    };
  ...
  form(plugins = []) {
    ...
    const radioLocations = (label, displayN) => {
      const radioButton =
        this.profile.location === label
          ? yo`<div class="radio">
          <label for="${label}">
            <input type="radio" name="location" onclick="${e =>
              this.updateLoc(
                e
              )}" value="${label}" id="${label}" checked="checked" />${displayN}</label>
        </div>`
          : yo`<div class="radio">
          <label for="${label}">
            <input type="radio" name="location" onclick="${e =>
              this.updateLoc(
                e
              )}" value="${label}" id="${label}" />${displayN}</label>
        </div>`;
      return yo`<div>
        ${radioButton}
      </div>`;
    ...
    <form id="local-plugin-form">
      <div class="form-group">
        ...
        <input class="form-control" onchange="${e =>
          this.updateName(
            e
          )}" value="${name}" id="plugin-name" placeholder="Should be camelCase">
      </div>
      <div class="form-group">
        ...
        <input class="form-control" onchange="${e =>
          this.updateDisplayName(
            e
          )}" value="${displayName}" id="plugin-displayname" placeholder="Name in the header">
      </div>
      <div class="form-group">
        ...
        <input class="form-control" onchange="${e =>
          this.updateUrl(
            e
          )}" value="${url}" id="plugin-url" placeholder="ex: https://localhost:8000">
      </div>
      <div class="form-group">
