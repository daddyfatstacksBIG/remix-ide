...
module.exports = class CompilerImports extends Plugin {
  ...
  handleGithubCall(root, path, cb) {
    ...
    const token = globalRegistry
      .get("config")
      .api.get("settings/gist-access-token"); // TODO replace with the plugin call above https://github.com/ethereum/remix-ide/issues/2288
    ...
    return request.get(
      {
        url:
          "https://api.github.com/repos/" + root + "/contents/" + path + param,
        json: true
      },
      (err, r, data) => {
        if (err) {
